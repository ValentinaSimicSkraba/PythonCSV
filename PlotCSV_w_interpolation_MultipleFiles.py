...
def select_and_load_csv(prompt):
    ...
    if not file_path:
        ...
        return None
    return pd.read_csv(file_path, sep=';')  # Assuming CSVs are semicolon-separated
...
def compare_multiple_csvs():
    ...
    for i in range(num_files):
        df = select_and_load_csv(f"Select CSV file {i + 1}")
        if df is None: return
        x_col, y_col = choose_columns(df, f"File {i + 1}")
        dfs.append(df)
        x_cols.append(x_col)
        y_cols.append(y_col)

        if custom_legend_names:
            legends.append(input(f"Enter legend name for {y_col}: "))
        else:
    ...
    y_all = [y_base]  # Start with the base Y
